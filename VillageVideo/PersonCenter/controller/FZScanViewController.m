...
@interface FZScanViewController ()<AVCaptureMetadataOutputObjectsDelegate, UINavigationControllerDelegate, UIImagePickerControllerDelegate,FZMessageSocketDelegate>
...
@implementation FZScanViewController

- (void)viewDidLoad {
    [super viewDidLoad];
   [[FZMessageSocketMgr shareInstance] open];
    [FZMessageSocketMgr shareInstance].delegate = self;
    //初始化信息
    [self initInfo];
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection
{
    ...
    if ([metadataObjects count] > 0) {
        ...
        if([[[metadataObjects firstObject] stringValue] rangeOfString:@"http"].location !=NSNotFound)//_roaldSearchText
        {
            NSLog(@"yes");
            FZBaseWebViewController *vc = [[FZBaseWebViewController alloc]init];
            NSString *hhh = [NSString stringWithFormat:@"%@&uid=%@",[[metadataObjects firstObject] stringValue],[FZUserInformation shareInstance].userModel.uid];
            vc.hostUrl =hhh;
        ...
        {
            NSLog(@"no");
            [[FZMessageSocketMgr shareInstance] openMessageSocket];
            NSDictionary *dic = [self dictionaryWithJsonString:[[[metadataObjects firstObject] stringValue] stringByReplacingOccurrencesOfString:@"'" withString:@"\""]];
            NSString *uuid = [dic objectForKey:@"uuid"];
            NSDictionary *parm = @{@"uuid":uuid};
            
            [[FZMessageSocketMgr shareInstance] sendMessage:parm andmetho:@"arsyc_play_inited"];
        }
        if ([[[metadataObjects firstObject] stringValue] hasSuffix:@"http"]) {
