...
-(void)buttonOnClick:(UIButton *)sender{
    if (sender.tag == 200) {
        ...
        vc.hostUrl = [NSString stringWithFormat:@"%@/#/myCollection?userId=%@",App_JAIOXUYE_BASE_URL,self.userId];;
        vc.title = @"收藏的";
        [self.navigationController pushViewController:vc animated:YES];
    ...
    [FZNetworkingManager requestLittleAntMethod:@"getLittleVideoUserById" parameters:parm requestHandler:^(BOOL success, id  _Nullable response) {
        ...
        if (success == YES) {
            ...
            if ([model.vIP isEqualToString:@"1"]) {
                self.vipImageView.image = [UIImage imageNamed:@"会员"];
                self.vipTimeLabel.text = [NSString stringWithFormat:@"%@会员还有%ld天到期  ",@"  ",labs([self dateTimeDifferenceWithStartTime:dateString])];
            }else{
                self.vipImageView.image = [UIImage imageNamed:@"会员_act"];
                self.vipTimeLabel.text = [NSString stringWithFormat:@"%@当前未开通VIP会员  ",@"  "];
            }
        }
    }];
}
...
-(UIImageView *)headImageView
{
    if (_headImageView == nil)
    {
        ...
        [_headImageView addSubview:_vipImageView];
        _vipImageView.image = [UIImage imageNamed:@"会员_act"];
        
        _vipTimeLabel = [[UILabel alloc] init];
        _vipTimeLabel.font = [UIFont systemFontOfSize:10];
        
        _vipTimeLabel.textColor = [UIColor colorWithRed:255/255. green:255/255. blue:255/255. alpha:1.];
        _vipTimeLabel.textAlignment = NSTextAlignmentCenter;
        _vipTimeLabel.lineBreakMode = NSLineBreakByWordWrapping;
        [_vipTimeLabel setAdjustsFontSizeToFitWidth:YES];
        _vipTimeLabel.numberOfLines = 0;
        _vipTimeLabel.layer.masksToBounds = YES;
        _vipTimeLabel.layer.cornerRadius = 2;
        _vipTimeLabel.backgroundColor = [[UIColor whiteColor] colorWithAlphaComponent:0.3];
        _vipTimeLabel.text = @"  会员还有100天到期 ";
        [_headImageView addSubview:_vipTimeLabel];
...
- (UIViewController *)magicView:(VTMagicView *)magicView viewControllerAtPage:(NSUInteger)pageIndex
{
    ...
    if ([menuInfo isEqualToString:@"视频"]) {
        return self.ProductViewController;
         ...
         self.articleViewController.hostUrl=[NSString stringWithFormat:@"%@/#/myArticleList?userId=%@&uid=%@",App_JAIOXUYE_BASE_URL,[FZUserInformation shareInstance].userModel.uid,self.userId];
        ...
        self.bokokViewController.hostUrl=[NSString stringWithFormat:@"%@/#/topicWrongList?userId=%@&uid=%@",App_JAIOXUYE_BASE_URL,[FZUserInformation shareInstance].userModel.uid,self.userId];
//        self.bokokViewController.hostUrl=[NSString stringWithFormat:@"%@/#/topicWrongList?userId=33",@"http://192.168.50.186:7094"];
        return self.bokokViewController;
    }
    else if([menuInfo isEqualToString:@"主题任务"]){
         self.tasktViewController.hostUrl=[NSString stringWithFormat:@"%@/#/myThemeTask?userId=%@&targetId=%@&uid=%@",App_JAIOXUYE_BASE_URL,[FZUserInformation shareInstance].userModel.uid,@"1",self.userId];
...
//    _articleViewController.DDelegate = self;
    }
