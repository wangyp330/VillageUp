...
-(void)requestMusic{
    ...
    MusicData *effect = [[MusicData alloc] init];
    effect.name = @"原始";
    effect.iconPath = [[NSBundle mainBundle] pathForResource:@"无音乐" ofType:@"png"];
    effect.isSelected = YES;
    [self.musicAry addObject:effect];
    NSMutableDictionary *parm = [NSMutableDictionary new];
    [parm setObject:@(-1) forKey:@"pageNo"];
    [FZNetworkingManager requestLittleAntMethod:@"getVideoMusicList" parameters:parm requestHandler:^(BOOL success, id  _Nullable response) {
        NSLog(@"");
        if (success == YES) {
           
            [[response objectForKey:@"response"] enumerateObjectsUsingBlock:^(NSDictionary * obj, NSUInteger idx, BOOL * _Nonnull stop) {
                FZMusicmodel *model = [[FZMusicmodel alloc]initWithDictionary:obj error:nil];
                  MusicData *effect = [[MusicData alloc] init];
                effect.name = model.musicName;
                effect.eid = model.musicId;
                effect.musicPath = model.musicUrl;
                effect.iconPath = model.cover;
                 [self.musicAry addObject:effect];
            } ];
             ...
             dispatch_async(dispatch_get_main_queue(), ^{
                 ...
                 if (compressionEncoder.status == AVAssetExportSessionStatusCompleted)
                 {
                     dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                         ...
                         FZPublishShortViewController* cor = [[FZPublishShortViewController alloc] init];
                         cor.viderPath = compressionEncoder.outputURL;
//                         FZBaseNavigationViewController *na = [[FZBaseNavigationViewController alloc]initWithRootViewController:cor];
//                         [[AppDelegate appDelegate] pushViewController:cor animated:YES];
                         cor.viderType = self.viderType;
                         cor.targetId = self.targetId;
                         cor.tagModel = self.tagModel;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (collectionView == _collectionView) {
        ...
        cell.downLaodImgView.hidden = YES;
    ...
    }else if (collectionView == _stickersCollectionView)
    {
        ...
        if (indexPath.row == 0) {
           UIImage* image = [UIImage imageWithContentsOfFile:data.iconPath];
            ...
            [cell.iconImgView sd_setImageWithURL:[NSURL URLWithString:data.iconPath]];
           
            NSArray *paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
            NSString *music = [[paths objectAtIndex:0] stringByAppendingPathComponent:data.musicPath];
            BOOL isHave = [[NSFileManager defaultManager] fileExistsAtPath:music];
            if (isHave) {
                //存在
              cell.downLaodImgView.hidden = YES;
                
            }else{
               cell.downLaodImgView.hidden = NO;
            }
        }
        ...
        if (_rightLabelArray.count ==0) {
            ...
            if (indexPath.row ==0 ) {
                ...
                cell.iconImgView.layer.borderColor = APP_BLUE_COLOR.CGColor;
                cell.iconImgView.layer.borderWidth = 4;
                
            }else{
                
                cell.iconImgView.layer.borderColor = APP_BLUE_COLOR.CGColor;
                cell.iconImgView.layer.borderWidth = 0;
                
            }
            
        }else{
            
            if ([_rightLabelArray containsObject:self.musicAry[indexPath.item]]) {
...
//            FilterData* dataNow = [_filterAry objectAtIndex:indexPath.row];
        ...
        MusicItemCollectionViewCell *cell = (MusicItemCollectionViewCell *)[collectionView cellForItemAtIndexPath:indexPath];
//        [self updateCollectionViewCellStatus:cell selected:YES];
    
      _rightLabelArray = [NSMutableArray arrayWithObject:_musicAry[indexPath.item]];
        ...
        }else
        {
            ...
            if (isHave) {
                //存在
                NSString *localMusicPath = music;
                _audioPath = localMusicPath;
                [self playMusic];
                _editTheOriginaBtn.hidden = NO;
                _editTheOriginaSwitch.hidden = NO;
            ...
            [[FZNetworkingManager sharedUtil] downloadFileWithUrl:data.musicPath withSaveFilePath:music callbackToMainThread:YES complete:^(BOOL success, id response, NSError *error) {
                cell.downLaodImgView.hidden = YES;
//                [self.musicCollectionView reloadItemsAtIndexPaths:@[indexPath]];
                [collectionView reloadData];
                [SVProgressHUD hidGif];
                self->_audioPath = music;
                [self playMusic];
                self->_editTheOriginaBtn.hidden = NO;
                self->_editTheOriginaSwitch.hidden = NO;
            }];
            }
        [collectionView reloadData];
        }
...
//-(void)updateCollectionViewCellStatus:(MusicItemCollectionViewCell *)myCollectionCell selected:(BOOL)selected
//{
//    myCollectionCell.iconImgView.layer.borderColor = selected ? APP_BLUE_COLOR.CGColor:[UIColor clearColor].CGColor;
//    myCollectionCell.iconImgView.layer.borderWidth = selected ? 4:1.0;
