...
-(void)requestdATA{
    ...
    if (_type == 1) {
       //村
       [parm setObject:[FZUserInformation shareInstance].userModel.villageId forKey:@"vid"];
        method=@"getLittleVideoByVillage";
    }else{
        //组
         [parm setObject:[FZUserInformation shareInstance].userModel.groupId forKey:@"gid"];
        method=@"getLittleVideoByVillageGroup";
    }
    [FZNetworkingManager requestLittleAntMethod:method parameters:parm requestHandler:^(BOOL success, id  _Nullable response) {
