...
@property (nonatomic, strong) FZRecommendViewController *villageViewController;
@property (nonatomic, strong) FZRecommendViewController *groupViewController;
...
-(void)rightButton{
    ...
    VC.hostUrl = [NSString stringWithFormat:@"%@/#/searchHistory?uid=%@",PhoneWebIP,[FZUserInformation shareInstance].userModel.uid];
...
- (UIViewController *)magicView:(VTMagicView *)magicView viewControllerAtPage:(NSUInteger)pageIndex {
    ...
    if ([menuInfo isEqualToString:@"本村"]) {
        if (!_villageViewController) {
            _villageViewController = [[FZRecommendViewController alloc] init];
        }
        _villageViewController.type = 1;
        return _villageViewController;
    } else{
        if (!_groupViewController) {
            _groupViewController = [[FZRecommendViewController alloc] init];
        }
        _groupViewController.type = 2;
        return _groupViewController;
    }
}
