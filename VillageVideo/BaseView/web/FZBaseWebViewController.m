...
-(BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType{
    ...
    }else if ([requestString hasPrefix:@"myweb:showLoadingClick"])
    {
        ...
        if(err) {
            ...
            if ([method isEqualToString:@"operateStructure"])
            {
                //组织架构业务
            }
            if ([method isEqualToString:@"waterDetailMethod"])
            {
                //组织架构业务
            }
            if ([method isEqualToString:@"operateStructureRole"])
            {
                //角色组业务
            }
            if ([method isEqualToString:@"showCloudFileShare"]) {
                //蚁盘文件分享
            }
...
//                [self loadWebView];
            ...
            }if ([method isEqualToString:@"changeOwener"]) {
               
            }if ([method isEqualToString:@"ringBinding"]) {
